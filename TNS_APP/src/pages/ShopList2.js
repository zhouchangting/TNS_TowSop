import React from 'react'
import '../assets/js/flexible'
import '../assets/css/reset.css'
// import '../assets/css/ShopList2.scss'

class ShopList2 extends React.Component {
    render () {
        return (
            <div className="shoplist2Box">
                <header>
                    <a className="left iconfont icon-zuo"></a>
                    <div className="searchBox">
                        <div className="search_box">
                            <form className="searchshop">
                                <input type="text" placeholder="搜索你想要的商品"/>
                                <span className="iconfont icon-fangdajing"></span>
                            </form>
                        </div>
                    </div>
                    <div className="rightBox">
                        <a>
                            <span className="iconfont icon-gouwuche"></span>
                        </a>
                        <a>
                            <span className="iconfont icon-fangzi"></span>
                        </a>
                    </div>
                </header>
                <div className="container">
                    <div className="change-two">
                        <ul>
                            <li className="white">
                                <i className="iconfont icon-liebiao"></i>
                                列表视图
                            </li>
                            <li>
                                <i className="iconfont icon-leimupinleifenleileibie"></i>
                                图片视图
                            </li>
                        </ul>
                    </div>
                    <div className="big-change">
                        <div className="big-change-main">
                            <div className="item-box">
                                <a>
                                    <span>分类</span>
                                    <i></i>
                                </a>
                                <a>
                                    <span>品牌</span>
                                    <i></i>
                                </a>
                                <a>
                                    <span>综合排序</span>
                                    <i></i>
                                </a>
                                <span className="item-JD">
                                    <input type="checkbox"/>
                                    <label>京东配送</label>
                                </span>
                            </div>
                            <ul>
                                <li className="iconfont icon-shaixuan"></li>
                            </ul>
                        </div>
                    </div>
                    <div className="openList">
                        <ul>
                            <li>
                                <div className="item">
                                    <div className="pic-box">
                                        <a>
                                            <img src="https://img12.360buyimg.com/n7/s230x230_jfs/t1/146504/32/2696/373004/5f09e3f3E51c83296/d8a1391ba6f566e0.jpg"/>
                                        </a>
                                    </div>
                                    <div className="title">
                                        <div className="title-box">
                                            三只松鼠每日坚果大礼包 孕妇零食节日礼物送女友混合干果礼盒榛子腰果巴旦木葡萄干核桃仁开心果 750g/30袋
                                        </div>
                                    </div>
                                    <div className="price">
                                        <span className="newprice">
                                            <small>￥</small>
                                            <i>149</i>
                                        </span>
                                        <span className="icon iconfont icon-shangpinjiarugouwuche">
                                            
                                        </span>
                                    </div>
                                    <div className="comment-shop">
                                        <span className="comment-one">
                                            <i className="iconfont icon-pinglun"></i>
                                            298万+
                                        </span>
                                        <span className="comment-two">
                                            <i className="iconfont icon-shangpinjiarugouwuche"></i>
                                            98%
                                       </span>
                                    </div>
                                    <div className="search-shop">
                                        <span className="small">自营</span>
                                        <a>
                                           <span className="store">三只松鼠自营旗舰店&gt;</span> 
                                        </a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div className="item">
                                    <div className="pic-box">
                                        <a>
                                            <img src="https://img12.360buyimg.com/n7/s230x230_jfs/t1/146504/32/2696/373004/5f09e3f3E51c83296/d8a1391ba6f566e0.jpg"/>
                                        </a>
                                    </div>
                                    <div className="title">
                                        <div className="title-box">
                                            三只松鼠每日坚果大礼包 孕妇零食节日礼物送女友混合干果礼盒榛子腰果巴旦木葡萄干核桃仁开心果 750g/30袋
                                        </div>
                                    </div>
                                    <div className="price">
                                        <span className="newprice">
                                            <small>￥</small>
                                            <i>149</i>
                                        </span>
                                        <span className="icon iconfont icon-shangpinjiarugouwuche">

                                        </span>
                                    </div>
                                    <div className="comment-shop">
                                        <span className="comment-one">
                                            <i className="iconfont icon-pinglun"></i>
                                            298万+
                                        </span>
                                        <span className="comment-two">
                                            <i className="iconfont icon-shangpinjiarugouwuche"></i>
                                            98%
                                       </span>
                                    </div>
                                    <div className="search-shop">
                                        <span className="small">自营</span>
                                        <a>
                                           <span className="store">三只松鼠自营旗舰店&gt;</span>
                                        </a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div className="item">
                                    <div className="pic-box">
                                        <a>
                                            <img src="https://img12.360buyimg.com/n7/s230x230_jfs/t1/146504/32/2696/373004/5f09e3f3E51c83296/d8a1391ba6f566e0.jpg"/>
                                        </a>
                                    </div>
                                    <div className="title">
                                        <div className="title-box">
                                            三只松鼠每日坚果大礼包 孕妇零食节日礼物送女友混合干果礼盒榛子腰果巴旦木葡萄干核桃仁开心果 750g/30袋
                                        </div>
                                    </div>
                                    <div className="price">
                                        <span className="newprice">
                                            <small>￥</small>
                                            <i>149</i>
                                        </span>
                                        <span className="icon iconfont icon-shangpinjiarugouwuche">

                                        </span>
                                    </div>
                                    <div className="comment-shop">
                                        <span className="comment-one">
                                            <i className="iconfont icon-pinglun"></i>
                                            298万+
                                        </span>
                                        <span className="comment-two">
                                            <i className="iconfont icon-shangpinjiarugouwuche"></i>
                                            98%
                                       </span>
                                    </div>
                                    <div className="search-shop">
                                        <span className="small">自营</span>
                                        <a>
                                           <span className="store">三只松鼠自营旗舰店&gt;</span>
                                        </a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div className="item">
                                    <div className="pic-box">
                                        <a>
                                            <img src="https://img12.360buyimg.com/n7/s230x230_jfs/t1/146504/32/2696/373004/5f09e3f3E51c83296/d8a1391ba6f566e0.jpg"/>
                                        </a>
                                    </div>
                                    <div className="title">
                                        <div className="title-box">
                                            三只松鼠每日坚果大礼包 孕妇零食节日礼物送女友混合干果礼盒榛子腰果巴旦木葡萄干核桃仁开心果 750g/30袋
                                        </div>
                                    </div>
                                    <div className="price">
                                        <span className="newprice">
                                            <small>￥</small>
                                            <i>149</i>
                                        </span>
                                        <span className="icon iconfont icon-shangpinjiarugouwuche">

                                        </span>
                                    </div>
                                    <div className="comment-shop">
                                        <span className="comment-one">
                                            <i className="iconfont icon-pinglun"></i>
                                            298万+
                                        </span>
                                        <span className="comment-two">
                                            <i className="iconfont icon-shangpinjiarugouwuche"></i>
                                            98%
                                       </span>
                                    </div>
                                    <div className="search-shop">
                                        <span className="small">自营</span>
                                        <a>
                                           <span className="store">三只松鼠自营旗舰店&gt;</span>
                                        </a>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        )
    }
}

export default ShopList2