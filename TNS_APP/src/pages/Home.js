...
function Home (props){
    let [superShop,superShopCh] = useState([])
    let [superNowTime,superNowTimeCh] = useState({ //倒计时
        hour : '00',
        minute : '00',
        second : '00',
    })
    let {goShopList,goOneShop} = useCallback({
        goShopList(listType){
            props.history.push({
                pathname:'/shopList',
                search:`?type=${listType}`
            })
        },
        goOneShop(shopId){
            props.history.push({
    ...
    useEffect(()=>{
        shopData.shopData().then(res=>{
            if(res.code == '2000'){
                let superShop = []
                for(let i = 0; i < res.data.length; i++){
                    if(res.data[i].typeSpecification.length == 0)continue
                    // console.log(res.data[i].typeSpecification.specification)
                    // if(!res.data[i].typeSpecification.specification)continue
                    // if(res.data[i].typeSpecification.specification.length == 0)continue
                    superShop.push({
                        title:res.data[i].goodstitle,
                    ...
                    if(i > 3) {
                        superShopCh(superShop)
                        break
                    }
                }
            }
        })
        let superTime = setInterval(()=>{
            let date = 1595751490000 - new Date().getTime()
            let hour = parseInt(date / 1000 / 60 / 60 ) + ''
            let minute = parseInt((date - hour * 3600000) / 1000 / 60) + ''
            let second = parseInt((date - hour * 3600000 - minute * 60000) / 1000) + ''
                ...
                <div className="superonce">
                    <div className="rush-head">
                        ...
                        <div className="last-box">
                            <span className="last-time">
                                <em className="hour num">{superNowTime.hour}</em>
                                <em className="split">:</em>
                                <em className="min num">{superNowTime.minute}</em>
                                <em className="split">:</em>
                                <em className="sec num">{superNowTime.second}</em>
                            </span>
                        </div>
                    ...
                    <div className="rush-box">
                        <div className="product-list">
                            ...
                            {/*</div>*/}
                            {
                                superShop.map((item,index)=>(
                                    <div className="product-item" key={index} onClick={()=>goOneShop(item.goods_id)}>
                                        <a className="product-box">
                                            <div className="pic-wrap">
                                                <img src={item.imgsrc} />
                                            </div>
                                            ...
                                            <div className="price-box">
                                                <div className="now-price">
                                                    <span>￥{item.discount * item.price}</span>
                                                </div>
                                                <div className="old-price">
